 ...
 */
//patched from https://github.com/raml-org/raml-java-parser
//remove when https://github.com/raml-org/raml-java-parser/issues/80 is solved
//changed method getTemplateParameters on handle include tags
...
class IncludeAwareTemplateResolver extends TemplateResolver {
    ...
    public IncludeAwareTemplateResolver(ResourceLoader resourceLoader, NodeHandler nodeNandler) {
        super(resourceLoader, nodeNandler);
        ...
        private Map<String, String> getTemplateParameters(Node node, Map<String, String> parameters) {
            if (node.getNodeId() == mapping) {
                ...
                for (NodeTuple paramTuple : ((MappingNode) params).getValue()) {
                    ...
                    String paramValue;
                    if (paramTuple.getValueNode().getTag().startsWith("!include")) {
                        final IncludeResolver ir = new IncludeResolver();
                        final ContextPath contextPath = new ContextPath();
                        contextPath.push(new Tag("!include_applied_bla_1_2_3"));
                        ir.setContextPath(contextPath);
                        final Node resolve = ir.resolve(paramTuple.getValueNode(), resourceLoader, nodeNandler);
                        paramValue = ((ScalarNode) resolve).getValue();
                    }else {
                        paramValue = ((ScalarNode) paramTuple.getValueNode()).getValue();
                    }
